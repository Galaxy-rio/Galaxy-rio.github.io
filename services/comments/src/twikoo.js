...
function protect (request) {
  ...
  const now = Date.now()
  let window = requestTimes.get(ip)
  if (!window || now >= window.expires) {
    if (requestTimes.size >= 4096) {
      requestTimes.delete(requestTimes.keys().next().value)
    }
    window = { count: 0, expires: now + 60000 }
    requestTimes.set(ip, window)
  }
  if (++window.count > MAX_REQUEST_TIMES) throw new Error('Too Many Requests')
}
